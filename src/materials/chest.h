...
{
public:
  Chest(const glm::vec3 &minBound, const glm::vec3 &maxBound, const Material &mat)
  ...
  Intersect rayIntersect(const glm::vec3 &rayOrigin, const glm::vec3 &rayDirection) const override
  {
    ...
    // Añadir textura arriba
      Color c = loadTexture(std::abs(intersect.point.x - minBound.x), std::abs(intersect.point.z - minBound.z), "chest_top");
      ...
      // caras z
      Color c = loadTexture(std::abs(intersect.point.x - minBound.x), std::abs(intersect.point.y - minBound.y), "chest_front");
      ...
      // caras x
      Color c = loadTexture(std::abs(intersect.point.z - minBound.z), std::abs(intersect.point.y - minBound.y), "chest_side");
